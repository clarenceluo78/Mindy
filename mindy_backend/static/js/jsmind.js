...
;(function ($w) {
    ...
    jm.prototype = {
        ...
        add_node: function (parent_node, nodeid, topic, data, stop_invoke_event) {
            if (this.get_editable()) {
        ...
        insert_node_before: function (node_before, nodeid, topic, data, stop_invoke_event) {
            if (this.get_editable()) {
        ...
        insert_node_after: function (node_after, nodeid, topic, data, stop_invoke_event) {
            if (this.get_editable()) {
        ...
        remove_node: function (node, stop_invoke_event) {
            if (!jm.util.is_node(node)) {
                var the_node = this.get_node(node);
                if (!the_node) {
                    logger.error('the node[id=' + node + '] can not be found.');
        ...
        update_node: function (nodeid, topic, stop_invoke_event) {
            if (this.get_editable()) {
                ...
                if (!!node) {
                    ...
                    this.auto_height_node_before_update(node)
        ...
        auto_height_node_before_update: function (node) {
            ...
            if (!!topic) {
                ...
                const height = (lines * 12 + 14)
                element.style.height = `${height}px`
            }
        },

        move_node: function (nodeid, beforeid, parentid, direction, stop_invoke_event) {
            if (this.get_editable()) {
    ...
    jm.view_provider.prototype = {
        init: function () {
            ...
            this.e_editor.style.fontSize = '11px';
        ...
        edit_node_begin: function (node) {
            ...
            this.e_editor.style.height = (element.clientHeight - parseInt(ncs.getPropertyValue('padding-top')) - parseInt(ncs.getPropertyValue('padding-bottom'))) + 'px';
            element.innerHTML = '';
