...
from django.core.paginator import Paginator, PageNotAnInteger, EmptyPage  # 后端分页
...
from django.views.decorators.http import require_http_methods  # 视图请求方法装饰器
...
from app_admin.models import UserOptions, SysSetting, UserImage
...
def get_pro_toc(pro_id):
    ...
    n = 0
    # 获取一级文档
    top_docs = Doc.objects.filter(top_doc=pro_id, parent_doc=0, status=1).values('id', 'name', 'open_children',
                                                                                 'editor_mode').order_by('sort')
    # 遍历一级文档
    for doc in top_docs:
        top_item = {
            'id': doc['id'],
...
def project_list(request):
    ...
    # 没有搜索 and 认证用户 and 有筛选
        ...
        if role in ['0', 0]:
            ...
            colla_list = [i.project.id for i in ProjectCollaborator.objects.filter(user=request.user)]  # 用户的协作文集列表
            project_list = Project.objects.filter(id__in=colla_list).order_by('-is_top',
...
def create_project(request):
    try:
        ...
        if name != '':
            ...
            project.save()
            return JsonResponse(
                {'status': True, 'data': {'id': project.id, 'name': project.name, 'role': project.role}})
        else:
...
def project_index(request, pro_id):
    ...
    try:
        ...
        if kw != '':
            search_result = Doc.objects.filter(Q(pre_content__icontains=kw) | Q(name__icontains=kw),
                                               top_doc=int(pro_id))
        ...
        if doc_id:
            if range == 'single':
                ...
                if (request.user == doc.create_user) \
                    ...
                    Doc.objects.filter(parent_doc__in=list(chr_doc_ids)).update(status=3,
                ...
                try:
                    ...
                    if request.user.is_superuser:
                        ...
                        Doc.objects.filter(id__in=docs, create_user=request.user).update(status=3,
                                                                                         modify_time=datetime.datetime.now())
                        Doc.objects.filter(parent_doc__in=docs).update(status=3, modify_time=datetime.datetime.now())
                    return JsonResponse({'status': True, 'data': _('删除完成')})
                except:
...
def manage_doc(request):
    if request.method == 'GET':
        ...
        if project == '':
            ...
            colla_project_list = ProjectCollaborator.objects.filter(user=request.user).values_list('project__id',
                                                                                                   flat=True)  # 协作的文集列表
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # 私密分享
                ...
                share_value = request.COOKIES.get(
                    share_cookie_name) if share_cookie_name in request.COOKIES.keys() else 0
...
def get_pro_doc(request):
    ...
    if pro_id != '':
        ...
        doc_list = Doc.objects.filter(top_doc=int(pro_id), status=1).values_list('id', 'name', 'parent_doc').order_by(
            'parent_doc')
        item_list = []
        # 遍历文档
        for doc in doc_list:
...
def get_pro_doc_tree(request):
    ...
    if pro_id:
        ...
        doc_list = []
        # 获取一级文档
        top_docs = Doc.objects.filter(top_doc=pro_id, parent_doc=0, status=1).values('id', 'name',
                                                                                     'modify_time').order_by('sort')
        # 遍历一级文档
        for doc in top_docs:
            top_item = {
                'id': doc['id'],
            ...
            if doc['id'] in parent_id_list:
                # 获取二级文档
                sec_docs = Doc.objects.filter(top_doc=pro_id, parent_doc=doc['id'], status=1).values('id', 'name',
...
def manage_image(request):
    ...
    if request.method == 'GET':
        try:
            groups = ImageGroup.objects.filter(user=request.user)  # get all groups, e.g.User
            all_img_cnt = Image.objects.filter(user=request.user).count()  # get image count
            no_group_cnt = Image.objects.filter(user=request.user, group_id=None).count()  # get ungrouped image count
            g_id = int(request.GET.get('group', 0))  # group id
            # 0 indicates all group
            if int(g_id) == 0:
                image_list = Image.objects.filter(user=request.user).order_by('-create_time')
            # -1 indicates ungrouped
            elif int(g_id) == -1:
                image_list = Image.objects.filter(user=request.user, group_id=None).order_by('-create_time')
            # others indicate specific group
            else:
                image_list = Image.objects.filter(user=request.user, group_id=g_id).order_by('-create_time')
            # apply paginator
            ...
            logger.exception(_("Error in image management!"))
            return render(request, '404.html')

    elif request.method == 'POST':
        try:
            img_id = request.POST.get('img_id', '')
            types = request.POST.get('types', '')  # operation type: 0-delete，1-modify，2-get
            range = request.POST.get('range', ' single')  # operation range: single and multiple pictures
            # delete
            if int(types) == 0:
                if range == 'single':
                    ...
                    if is_exist:
                        os.remove(file_path)
                    img.delete()  # delete log
                elif range == 'multi':
                    imgs = img_id.split(',')
                    for i in imgs:
                        img = Image.objects.get(id=i)
                        if img.user != request.user:
                            logger.error(_("Picture{} illegal deleted".format(i)))
                            break
                        file_path = settings.BASE_DIR + img.file_path
                        is_exist = os.path.exists(file_path)
                        if is_exist:
                ...
                return JsonResponse({'status': True, 'data': _('Done')})
            # modify image group
            elif int(types) == 1:
                if range == 'single':
            ...
            # get image
                ...
                group_id = request.POST.get('group_id', None)  # get group id
                if group_id is None:  #
                    return JsonResponse({'status': False, 'data': _('id error!')})
                elif int(group_id) == 0:
...
def manage_self(request):
    if request.method == 'GET':
        ...
        # userimage = UserImage(user.username, image)
        # userimage.save()

        if User.objects.filter(email=email).count() > 0 and user.email != email:
            return JsonResponse({'status': False, 'data': _('Email has been occupied!')})
        # post success
        if email != '' and '@' in email:
            ...
            return JsonResponse({'status': False, 'data': _('Wrong input!')})
...
def my_collect(request):
    if request.method == 'GET':
        ...
        if (collect_type is None) or (collect_id is None):
            ...
            is_collect = MyCollect.objects.filter(collect_type=collect_type, collect_id=collect_id,
                                                  create_user=request.user)
            # 存在收藏
